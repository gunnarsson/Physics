...
public class Example4 extends BaseGameActivity implements
...
protected TiledTextureRegion tileRegions;
	...
	public void onLoadResources() {
		...
		this.tileRegions = TextureRegionFactory.createTiledFromAsset(
				this.texture, this, "gfx/jfokus.png", 0, 0, BLOCKS + 1, 1);
	...
	public Scene onLoadScene() {
		...
		for (int i = 0; i < BLOCKS; i++) {
			...
			// Create block
			TiledSprite block = new TiledSprite(x, 144,
			...
			this.physicsWorld.registerPhysicsConnector(new PhysicsConnector(
					block, blockBody, true, true, false, false));

			// Create anchor body
			Body anchorBody = PhysicsFactory.createBoxBody(physicsWorld,
					anchor, BodyType.StaticBody, fixtureDefinition);
			this.physicsWorld.registerPhysicsConnector(new PhysicsConnector(
					anchor, anchorBody));

			// Create joint
			final RevoluteJointDef revoluteJointDef = new RevoluteJointDef();
			revoluteJointDef.initialize(anchorBody, blockBody,
