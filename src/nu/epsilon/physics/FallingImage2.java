...
public class FallingImage2 extends BaseGameActivity implements
		...
		for (int i = 0; i < BLOCKS; i++) {
			for (int j = 0; j < BLOCKS; j++) {
				TiledSprite sprite = new TiledSprite(
						90 + i * (BLOCK_WIDTH + PADDING),
						-50 + j * (BLOCK_WIDTH+ PADDING), this.tileRegions.clone());
			...
			if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_DOWN) {
				this.runOnUpdateThread(new Runnable() {
					...
					public void run() {
						...
						FallingImage2.this.mEngine.getScene()
								.registerUpdateHandler(
										FallingImage2.this.physicsWorld);
					}
				});
				return true;
			}
