...
public class FreeFloatingImage extends BaseGameActivity implements
	...
	private static final int BLOCK_WIDTH = 512 / BLOCKS;
	...
	public void onLoadResources() {
		...
		this.texture = new Texture(512, 512, TextureOptions.BILINEAR);
		this.tileRegions = TextureRegionFactory.createTiledFromAsset(
				this.texture, this, "gfx/mattias2.jpg", 0, 0, BLOCKS, BLOCKS);
	...
	public Scene onLoadScene() {
		...
		for (int i = 0; i < BLOCKS; i++) {
			for (int j = 0; j < BLOCKS; j++) {
				TiledSprite sprite = new TiledSprite(90 + i
						* (BLOCK_WIDTH + PADDING), -50 + j
						* (BLOCK_WIDTH + PADDING), this.tileRegions.clone());
			...
			if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_DOWN) {
				this.runOnUpdateThread(new Runnable() {
					...
					public void run() {
						...
						FreeFloatingImage.this.mEngine.getScene()
								.registerUpdateHandler(
										FreeFloatingImage.this.physicsWorld);
					}
				});
				return true;
			}
